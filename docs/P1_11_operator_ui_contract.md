...
4. **Пользователь выбирает цель (сервис/игра) и источник трафика** (и, при необходимости, даёт согласие). Он не выбирает DoH/TLS/QUIC/стратегии.
...
### Шаг 1 — Выбор цели (сервис/игра)
- Доступно в `Idle` и после завершения (`Ok/Warn/Blocked`).
- В `Checking/Fixing` элементы выбора цели отключены.

Примечания:
- Выбор цели влияет на «контекст» диагностики/автопилота (OutcomeTargetHost) и отображается в истории Operator.
