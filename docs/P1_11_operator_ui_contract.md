...
- **Источник трафика**:
  ...
  - **Быстрая проверка интернета (тестовый режим)** — режим самопроверки UI/потока (не предназначен для диагностики результата). Доступен в настройках.
- **Автопилот** — разрешение на автоматическое применение безопасных и обратимых действий (стратегии не выбираются пользователем).
- **Разрешение DNS/DoH** — явное согласие на системные изменения DNS/DoH (может менять настройки сети Windows).
...
4. **Пользователь выбирает приложение (.exe)** (и, при необходимости, даёт согласие). Он не выбирает DoH/TLS/QUIC/стратегии.

### Согласие на системные изменения (DNS/DoH)

- В Operator UI есть явный переключатель **«Разрешить DNS/DoH»** (по умолчанию выключен).
- При первом включении переключателя показывается подтверждение **до выдачи согласия** (при Cancel согласие не сохраняется).
- Если INTEL‑план включает стратегию `UseDoh`, но разрешение не выдано:
  - Apply продолжает применять остальные стратегии/assist‑флаги.
  - Фаза DoH отображается как `apply_doh_skipped` («DoH: пропущено (нужно разрешение)»).
  - Реальные системные команды (FixService) не выполняются.
...
Примечание:
- Поле выбора приложения (.exe) отображается в основном потоке. «Быстрая проверка интернета» доступна только в настройках как тестовый режим.
...
- После завершения пост‑проверки основной статус возвращается в `Ok/Warn/Blocked` (по агрегированной семантике),
  но блок «Шаг 5» может оставаться как «последний итог исправления» до следующей проверки, чтобы оператор увидел результат.
