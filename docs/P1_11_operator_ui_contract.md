...
| `Fixing` | ApplyRunning | Исправляю… | Применяю безопасные действия и перепроверяю | (disabled) |
...
### Шаг 4 — Исправление
- Видим только в `Fixing`.
- После завершения — возвращаемся в `Ok/Warn/Blocked` (по итоговым счётчикам) + показываем результат.
