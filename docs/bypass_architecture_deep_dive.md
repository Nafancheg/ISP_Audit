...
Сейчас в проекте используются три слоя WinDivert (Multi-Layer Architecture):

1. **WINDIVERT_LAYER_FLOW** — в `FlowMonitorService` (анализ потоков):
   - цель: получить список **установленных** соединений процесса (кто с кем говорит, порты, протоколы);
   - флаги: `Sniff | RecvOnly`;
   - приоритет: `-1000` (пассивный мониторинг, чтобы не конфликтовать с Network слоем).

2. **WINDIVERT_LAYER_SOCKET** — в `FlowMonitorService` (анализ намерений):
   - цель: детектировать **попытки** подключения (`SOCKET_CONNECT`), даже если они блокируются фаерволом или DPI (когда Flow не создается);
   - флаги: `Sniff | RecvOnly`;
   - приоритет: `-1000`;
   - критически важен для обнаружения "черных дыр" (например, 1.1.1.1) и IPv6 трафика, который может не создавать Flow событий в некоторых конфигурациях.

3. **WINDIVERT_LAYER_NETWORK** — в `WinDivertBypassManager` (реальный обход) и `NetworkMonitorService` (DNS):
   - **DNS Monitor** (`NetworkMonitorService`):
     - фильтр: `udp.DstPort == 53 or udp.SrcPort == 53`;
     - приоритет: `-1000`;
     - цель: парсинг DNS ответов для обогащения логов (IP -> Hostname).
   - **TLS fragmenter**:
     - слой: `Network`;
     - фильтр: `"outbound and tcp.DstPort == 443 and tcp.PayloadLength > 0"`;
   ...
   - **RST blocker**:
     ...
     - фильтр: `"tcp.Rst == 1"`;
     - приоритет: `0`;
     - флаги: `Sniff | Drop` (или `0` с PumpPackets).
   - **Redirector**:
     - слой: `Network`;
     - приоритет: `0`.
...
**Реализованное решение (Вариант C + Socket Layer):**

Мы используем стратегию **разведения приоритетов** и **дополнительного слоя Socket**:

1. **Разведение приоритетов:**
   - Все диагностические хэндлы (`FlowMonitorService`, `NetworkMonitorService` для DNS) открываются с приоритетом **-1000**.
   - Это гарантирует, что они видят трафик "как есть" (или "как было"), но не вмешиваются в работу активных модулей обхода (которые работают на приоритетах 0..200).
   - Драйвер WinDivert обрабатывает пакеты от высшего приоритета к низшему (для outbound) или наоборот (для inbound), но Flow/Socket события генерируются параллельно. Низкий приоритет минимизирует риск того, что диагностика "украдет" пакет у RST-блокера.

2. **Socket Layer для надежности:**
   - Flow Layer иногда пропускает короткоживущие соединения или те, что были заблокированы на этапе SYN (RST сразу).
   - Добавление `WINDIVERT_LAYER_SOCKET` позволяет видеть намерения приложения (`connect()`), даже если сеть полностью заблокирована.
   - Это решает проблему "невидимых" блокировок (когда 1.1.1.1 недоступен, и Flow не создается).

**Альтернативы (отвергнутые или запасные):**

**Вариант A (жёсткий):**
- Отключать Flow во время Bypass.
- *Статус:* Оставлен как fallback, если приоритеты не помогут.

**Вариант B (мягкий):**
- Тонкая игра с флагами `Sniff|Drop`.
- *Статус:* Слишком хрупко и зависит от версии драйвера.

**Вариант C (приоритетное разведение Flow и Network):**
- *Статус:* **Внедрено**. Flow = -1000, Bypass = 0..200.
