<!-- markdownlint-disable MD007 MD022 MD032 MD040 MD056 MD060 -->
...
- P1.9 (UX): при наличии win для цели Operator UI применяет wins-план одной кнопкой (без INTEL-подбора/вопросов) и сразу запускает пост-Apply ретест.
- P1.10 (UX): если пост-Apply ретест дал `FAIL/PARTIAL`, Operator UI переключает CTA на **«Усилить»** и применяет следующую ступень обхода (эскалация) с последующим пост-Apply ретестом.
...
Актуализация (Dev, 13.02.2026): добавлен DI контейнер (composition root в `App` через `Microsoft.Extensions.DependencyInjection`), регистрации вынесены в `Utils/ServiceCollectionExtensions.cs`. Static singleton API `NoiseHostFilter.Initialize/Instance` удалён; `NoiseHostFilter`/`ITrafficFilter` (`UnifiedTrafficFilter`) и `AutoHostlistService` стали shared singleton-сервисами. Убраны скрытые fallback-конструкторы/`?? new ...` в runtime-пайплайне: `TrafficCollector` и `LiveTestingPipeline` требуют явный `ITrafficFilter`. Затем DI-граф расширен до core/VM-слоя: `TrafficEngine` + `BypassStateManager` (SSoT сохраняется через `BypassStateManager.GetOrCreate`), а также `BypassController`/`DiagnosticOrchestrator`/`TestResultsManager`/`GroupBypassAttachmentStore` — теперь создаются через DI, и `MainViewModel` не конструирует их вручную. У `DiagnosticOrchestrator`/`TestResultsManager` убраны fallback-пути, которые создавали `NoiseHostFilter` внутри. Дополнительно XAML-конвертеры больше не создают `NoiseHostFilter` вручную: контейнер инициализируется до загрузки ресурсов, а доступ к фильтру идёт через DI (best-effort).
