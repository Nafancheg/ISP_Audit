...
Дополнение (06.02.2026): P0.3 — приложение пишет best-effort crash-report для необработанных исключений (UI thread / AppDomain / UnobservedTaskException) в `state\\crash_reports\\app\\`.

Дополнение (06.02.2026): P1.4 — post-crash UX: при запуске (GUI) показывается баннер «Отчёты о падении» при наличии новых crash-report JSON (app + traffic_engine). Отметка «просмотрено» хранится в `state\\crash_reports_seen.json`.

Дополнение (02.02.2026): в UI добавлен режим **"Фокус"** для таблицы результатов (чтобы быстро видеть только проблемные/активные/применённые строки) и компактная сводка "что применено" по текущей активной группе (из apply-транзакций) + статус outcome-probe.

Дополнение (03.02.2026): P1.9 — в UI-результатах добавлена агрегация подхостов/CDN по groupKey. В модели `TestResult` появился детерминированный ключ `UiKey`, отображается бейдж `×N` (число схлопнутых хостов), а привязка INTEL-рекомендаций (BypassStrategy) учитывает `UiKey`, чтобы стратегия применялась к агрегированной карточке.

Дополнение (03.02.2026): P1.10 — история Apply во вкладке «Применение» дедуплицируется по `(GroupKey + PlanText)`: показывается последний Apply и счётчик повторов `×N`, при этом для аудита доступен полный список транзакций группы в JSON выбранной строки.
...
Дополнение (10.02.2026): P1.11 (Operator UX) — в Operator убран ручной выбор «цели (сервис/игра)» (нет поддерживаемых профилей/каталога сервисов). В основном потоке остаётся выбор приложения (.exe), а «проблемный ресурс» показывается как производный якорь на базе `OutcomeTargetHost`.

Дополнение (10.02.2026): P1.11 (Operator UX) — на экране итога по умолчанию показывается сводка «проверено N / проблемных M» и список проблем карточками; «сырые» детали и история остаются под Expander «Подробнее».
...
Дополнение (04.02.2026): P1.11 — в Operator UI оформлен явный «шаг 5» wizard: исправление (Apply) → пост‑проверка (ретест) → краткий итог (стало лучше/частично/не помогло/не удалось проверить).

Дополнение (09.02.2026): P1.11 — добавлен минималистичный header Operator UI (⚙️ настройки + ? справка) и два operator-safe окна: `OperatorSettingsWindow` и `OperatorHelpWindow`.
...
Дополнение (05.02.2026): чтение и парсинг ENV (trim/bool/int) централизованы в `Utils/EnvVar.cs`, чтобы убрать дублирование и снизить риск расхождений.
