...
Дополнение (10.02.2026): P1.11 (Operator UX) — добавлен шаг выбора цели (сервис/игра): «популярные/последние» плитки + второстепенная кнопка «Обзор…» (полный список). Выбор сохраняется best-effort в `state\\operator_target.json` и выставляет `OutcomeTargetHost` для контекста автопилота/селективных техник.
