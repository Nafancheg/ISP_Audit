...
- TLS-обход в `BypassFilter` применим только к TCP-трафику (ClientHello на 443). YouTube и многие сайты по умолчанию используют QUIC/HTTP3 (UDP/443), поэтому «TLS стратегии включены, а сайт не оживает» может быть просто тем, что трафик не TCP.
- Для пресета `v2` добавлено детерминированное поведение: исходящий UDP на порт 443 отбрасывается (`Udp443Dropped++`), чтобы браузер гарантированно переключился на TCP/HTTPS и TLS-обход мог сработать.
