...
| `TestResultsManager` | Парсинг + хранение + здоровье + UI обновления | после P0.3: partial-файлы 63/35/46/136/606/89/71/327 (~1373 строк суммарно) |
...
| `ViewModels/TestResultsManager*.cs` | 🟡 Средний | Уже разбит на `partial`; следующий шаг (опционально) — выделить парсер сообщений pipeline в отдельный класс |
