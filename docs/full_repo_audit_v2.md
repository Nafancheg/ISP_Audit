...
- В v2 контуре assist-флаги могут попадать в рекомендацию как токены `DROP_UDP_443` / `ALLOW_NO_SNI` и применяются при ручном `ApplyV2PlanAsync`.
    Важно: DNS/DoH считается "верхнеуровневым" системным изменением и по умолчанию **не применяется автоматически** из v2 рекомендаций
