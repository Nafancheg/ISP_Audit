...
- В v2 контуре эти assist-флаги могут попадать в рекомендацию как токены `DROP_UDP_443` / `ALLOW_NO_SNI`.
    Важно: по умолчанию они **не применяются автоматически** при ручном `ApplyV2PlanAsync` (архитектурный гейт для наблюдаемости);
    исполнение в `Core/Bypass/BypassApplyService` поддерживает включение assist только в инженерном режиме через runtime feature gate.
