...
- Реализован P0.2 Этап 5 (частично, каркас): введены Semantic Groups как «пакет политик» и детерминированная оценка статуса группы `NO_TRAFFIC / PARTIAL / ENABLED` на основе per-policy метрики `matched_count` (в дополнение к `applied_count`). Добавлены модель `SemanticGroup` и evaluator; smoke-гейт `DPI2-045`.
