...
- **Auto-hostlist (кандидаты)**: добавлен `AutoHostlistService`; `LiveTestingPipeline` на этапе классификации читает legacy `BlockageSignals` (ретрансмиссии/HTTP-редиректы/RST/UDP) только если Auto-hostlist включён, и пополняет список кандидатов, который показывается в UI (включается/выключается тумблером).
