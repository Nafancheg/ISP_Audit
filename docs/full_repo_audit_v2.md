...
- Практическое усиление (P0.0 hardening): `candidateIpEndpoints` сохраняются в `BypassStateManager.ActiveTargetPolicy` (best-effort) и используются как seed observed IPv4 перед компиляцией per-target политик (TCP/443, TCP/80), снижая зависимость от DNS resolve. Smoke-гейт: `DPI2-047`.
