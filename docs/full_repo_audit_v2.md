...
Актуализация (Runtime, 13.01.2026): гибридный доменный UX для YouTube CDN (`googlevideo.com`)
- На практике YouTube «лечится» через CDN-хосты вида `rr*-sn-*.googlevideo.com`, поэтому ручной apply по одному хосту создавал UX-хаос (много карточек/целей).
- Добавлен гибридный режим (MVP): при обнаружении нескольких shard-хостов UI может агрегировать их в доменную цель `googlevideo.com` и показывать отдельную кнопку «Подключить (домен: googlevideo.com)» в блоке рекомендаций.
- Доменное применение выполняется через `DiagnosticOrchestrator.ApplyRecommendationsForDomainAsync(..., "googlevideo.com")`: метод выбирает применимый v2-план из подхостов, но выставляет `OutcomeTargetHost` на домен.
