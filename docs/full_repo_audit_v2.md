...
- P0.1 (MVP, наблюдаемость): UI ведёт журнал транзакций «применения обхода» (hostKey + applied strategy + activation + policy snapshot) и показывает его в bypass-панели (вкладка «Применение») с экспортом JSON в `artifacts/` и копированием в буфер.
