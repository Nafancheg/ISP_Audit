...
- «Применение обхода» оформляется как транзакция с полным снимком (что применили, к какой карточке/hostKey, на какие endpoint-ы, какие assist-флаги/режимы/фильтры) и журналом.
- UI показывает не только факт применения, но и **владельца активной конфигурации** (какая карточка сейчас управляет обходом) — чтобы исключить ощущение «переключателя между карточками».
