...
**Дата**: 09.12.2025 (обновлено 10.12.2025)  
...
- `TrafficCollector` не создаёт UI-события для loopback и применяет `UnifiedTrafficFilter` до логирования, чтобы не оставлять «вечные» карточки для отброшенных целей.
...
- **Auto-hostlist (кандидаты)**: добавлен `AutoHostlistService`; `LiveTestingPipeline` на этапе классификации читает `BlockageSignals` (ретрансмиссии/HTTP-редиректы/RST/UDP) и пополняет список кандидатов, который показывается в UI (включается/выключается тумблером).
