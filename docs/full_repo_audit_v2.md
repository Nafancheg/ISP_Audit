...
| `TestResultsManager` | Парсинг + хранение + здоровье + UI обновления | после P0.3: partial-файлы 63/35/46/136/14/590/31/89/71/327 (~1402 строк суммарно) |
...
| `ViewModels/TestResultsManager*.cs` | 🟡 Средний | Уже разбит на `partial`; парсер pipeline вынесен в отдельный класс `PipelineMessageParser` (в файле `TestResultsManager.PipelineMessageParser.cs`). Следующий шаг (опционально) — сделать его top-level сервисом с явным контекстом |
