...
Примечание: имя файла историческое (`full_repo_audit_v2.md`). В пользовательском UX и в логах проекта больше не используется маркировка V1/V2; основной префикс рекомендаций — `[INTEL]` (старые `[V2]`/`v2:` поддерживаются только для обратной совместимости).
