...
> **ВАЖНО**: Данный подход актуален для **Stage2** (детальный анализ конкретных целей).  
> Для **Stage1** (поиск IP-адресов) используется упрощённый **Flow-only** режим, так как мониторинг запускается ДО старта приложения → все соединения отслеживаются через `FLOW_ESTABLISHED` события.
