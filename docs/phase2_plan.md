...
- Критерии:
    ...
    - Ограничения по риску — ответственность селектора: `RiskLevel.High` не предлагается при `confidence < 70`.
...
4. **Агрессивные стратегии (DROP_RST / `RiskLevel.High`) → только при confidence >= 70**
