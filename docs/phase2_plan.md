...
Правило пересечения (контракт):
- Если сработало несколько правил, выбирается правило с максимальным `BaseConfidence`.
- При равенстве `BaseConfidence` выбор должен быть **детерминированным** (например: порядок правил в списке).
- `MatchedRuleName` и `Notes` берутся от выбранного правила.
...
1) **Signals Adapter (логирование)**: собрать `BlockageSignals(v2)` из текущих данных (`HostTested` + state store), логировать и сравнить с реальностью.
  ...
  - Требования к стабильности (MVP): для одного и того же хоста при повторных проверках сигналы не должны «флапать».
    - Для булевых сигналов допускается не более 1 расхождения на 3 прогона (2/3 совпало).
    - Для численных метрик допускается “грубый коридор” (фиксируется в коде/логах как часть Step 1).
    - Если стабильность не достигается, сигнал помечается как `Unreliable` (в логах/выводе Step 1).
...
3) **Strategy Selector v2**: таблица `Diagnosis → Strategy + BasePriority`, выдаёт `BypassPlan`.
  - Критерий: план детерминированный, без доступа к Sensors.
  - Правило безопасности (MVP): при `Diagnosis=None/Unknown` или `Confidence < 50` возвращать **пустой** план.
  - В MVP таблица маппинга hardcoded (позже можно вынести, но без динамических условий).

Пример формата таблицы маппинга (MVP):
```
ActiveDpiEdge  -> [TlsDisorder(10), TlsFragment(8), TtlFake(5)]
DnsHijack      -> [UseDoh(10)]
None/Unknown   -> []
```
...
Gate 1→2 (контроль качества сигналов):
- Прогон: 10 хостов × 3 проверки = 30 измерений.
- Успех: минимум 8 из 10 хостов показывают стабильные сигналы (по правилам Step 1), логи объяснимы.
- Провал: нестабильность/шум > 20% → дорабатываем Step 1, на Step 2 не переходим.
