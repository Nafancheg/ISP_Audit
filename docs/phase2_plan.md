...
Примечание: в коде исторически используются имена `IntelligenceV2`, `StandardDiagnosisEngineV2`, `ApplyV2PlanAsync` и т.п. В пользовательском UX и в логах проекта основной маркер рекомендаций — `[INTEL]`.
...
и INTEL executor выставляет именно его, а не магический `"v2"`.
...
- Это не auto-apply: включение происходит только вручную пользователем (в том числе через «Применить рекомендации v2»).
