...
**Test ID:** `PERF-004`
**Что проверяет:** Производительность `DecisionGraphSnapshot.Evaluate()` при 100/500/1000 политиках
**Для чего:** Зафиксировать масштабирование policy-driven lookup при росте числа политик
**Критерий успеха:** Тест укладывается в мягкий time-budget (без флапа на dev-машинах)
**Входные данные:** Синтетические политики TCP/443 TLS ClientHello (худший кейс: почти полный проход списка кандидатов)
**Ожидаемый результат:** SmokeOutcome=Pass и вывод измеренных таймингов
...
**Test ID:** `PIPE-020`
**Что проверяет:** Приоритизация очереди pipeline (high preempts low)
**Для чего:** Чтобы manual retest/профильные цели не «тонул» в потоке фоновых соединений
**Критерий успеха:** High-priority цель начинает тестироваться < 5с при большом low-backlog
**Входные данные:** 60+ low элементов + 1 high элемент (MaxConcurrentTests=1)
**Ожидаемый результат:** high элемент стартует быстро (до истечения дедлайна)
...
**Test ID:** `UI-016`
**Что проверяет:** P1.11 — round-trip персиста «сессий» Operator (OperatorSessionStore)
**Для чего:** История Operator должна быть «списком сессий» и сохраняться/читаться без падений
**Критерий успеха:** Persist → Load возвращает сессии, новые сверху
...
**Test ID:** `UI-027`
**Что проверяет:** P1.8 — Operator UI: raw-коды проблем (например `TLS_AUTH_FAILURE`) не отображаются
**Для чего:** Чтобы оператор видел понятные формулировки вместо кодов классификатора/тестера
**Критерий успеха:** В `SummaryProblemCards` нет токенов `TLS_AUTH_FAILURE`/`TLS_DPI`/`TLS_TIMEOUT` при наличии проблемной карточки
**Входные данные:** Инъекция `TestResult` со строкой `Error="TLS_AUTH_FAILURE"` в `MainViewModel.TestResults`
**Ожидаемый результат:** Строки в Operator UI содержат локализованный текст (без raw-кодов)
...
**Test ID:** `ORCH-008`
**Что проверяет:** Сериализация операций (rapid Retest + Cancel)
**Для чего:** Устранить гонки Start/Cancel/Retest (P1.5)
**Критерий успеха:** Второй параллельный ретест завершается быстро (no-op), Cancel безопасно завершает активный ретест
**Входные данные:** Два параллельных вызова `RetestTargetsAsync` + `Cancel()`
**Ожидаемый результат:** Нет исключений/зависаний, `IsDiagnosticRunning=false` после завершения
...
**Ожидаемый результат:** `BypassProfile` корректно читается из файла и доступен для UI/сервисов
...
### 8.3 Domain Family Catalog

**Test ID:** `CFG-006`
**Что проверяет:** DomainFamily persist+reload + pinned/learned пороги
**Для чего:** UX доменного режима (агрегация карточек + доменный apply) должен быть стабильным между запусками
**Критерий успеха:** Каталог читается/пишется, pinned/learned пороги дают подсказку при минимальном числе подхостов
**Входные данные:** `state\\domain_families.json` (pinned+learned)
**Ожидаемый результат:** Подсказка доменного суффикса появляется по ожидаемым порогам

### 8.4 Domain Group Catalog

**Test ID:** `CFG-007`
**Что проверяет:** DomainGroups persist+reload + pinned подсказка
**Для чего:** Кросс-доменная группировка (например YouTube) должна быть стабильной и управляемой через внешний каталог
**Критерий успеха:** Каталог читается/пишется, pinned группа срабатывает на доменах из списка
...
**Test ID:** `CFG-009`
**Что проверяет:** DomainGroups learned ignore (не показывать подсказку)
**Для чего:** Пользователь должен уметь отключать ошибочные learned-подсказки без удаления файла/ручного редактирования
**Критерий успеха:** IgnoredLearnedGroupKeys влияет на анализатор, подсказка не появляется
**Входные данные:** Выученная learned-группа + добавление её ключа в ignore
**Ожидаемый результат:** Для host в `googlevideo.com` подсказка отсутствует

**Test ID:** `CFG-010`
**Что проверяет:** DomainGroups learned promote -> pinned
**Для чего:** Дать пользователю безопасный путь «подтвердить» learned-группу и сделать её стабильной (pinned)
**Критерий успеха:** Learned запись переносится в pinned и начинает срабатывать как pinned-группа
**Входные данные:** Выученная learned-группа + команда promote
**Ожидаемый результат:** После persist+reload подсказка становится pinned (GroupKey новой pinned-группы)
...
**Test ID:** `ERR-010`
**Что проверяет:** Fail-path отмены `FixService.ApplyDnsFixAsync`
**Для чего:** Гарантировать, что отмена операции DNS/DoH не приводит к падению
**Критерий успеха:** Возврат `success=false` и сообщение об отмене без исключения
**Входные данные:** Предварительно отменённый `CancellationToken`
**Ожидаемый результат:** Обработка cancel без crash

**Test ID:** `ERR-011`
**Что проверяет:** Fail-path запуска несуществующей команды в `FixService.RunCommandAsync`
**Для чего:** Гарантировать корректную обработку ошибок запуска процесса
**Критерий успеха:** Возврат `success=false` и текст ошибки без исключения
**Входные данные:** Заведомо несуществуемый executable
**Ожидаемый результат:** Ошибка процесса обработана без crash

**Test ID:** `ERR-012`
**Что проверяет:** Fail-path таймаута команды в `FixService.RunCommandAsync`
**Для чего:** Гарантировать, что зависшая команда завершается по timeout и не валит smoke
**Критерий успеха:** Возврат `success=false` с маркером `TIMEOUT`
**Входные данные:** Команда `Start-Sleep` с малым timeout
**Ожидаемый результат:** Таймаут обработан без crash
...
**Test ID:** `PERF-006`
**Что проверяет:** Latency hot path `TrafficEngine.ProcessPacketForSmoke` на 10K пакетах (p50/p95/p99)
**Для чего:** Базовый бенчмарк/раннее обнаружение катастрофической деградации на hot-path
**Критерий успеха:** Тест выполняется без исключений; p99 не превышает мягкий порог (см. Details)
**Входные данные:** 10K синтетических TCP пакетов (без WinDivert)
**Ожидаемый результат:** PASS + метрики p50/p95/p99 в Details
...
**Test ID:** `REG-018`
**Что проверяет:** HttpRedirect диагноз даёт непустой план обхода
**Для чего:** Устраняет расхождение «диагностируем HTTP redirect, но не реагируем». Минимальная реакция MVP — `HttpHostTricks` (TCP/80).
...
**Критерий успеха:** Все `ISP_AUDIT_*` ключи, которые реально читаются/пишутся через `Environment.Get/SetEnvironmentVariable(...)` (включая ключи, заданные через `const string`), перечислены в `docs/ENV_VARS.md`
**Входные данные:** Исходники репозитория + `docs/ENV_VARS.md`
**Ожидаемый результат:** Тест PASS, отсутствует список "missing"

**Test ID:** `REG-024`
**Что проверяет:** Повторный Apply для того же домена не выполняет повторное применение (dedup)
**Для чего:** P1.1: защита от «apply storm» при повторных кликах по одинаковой цели/домену
**Критерий успеха:** Первый вызов Apply возвращает `Status=APPLIED`, второй — `Status=ALREADY_APPLIED`
**Входные данные:** `DiagnosticOrchestrator` + сохранённый план для поддомена (`sub.example.com`) и вызовы `ApplyRecommendationsForDomainAsync(..., "example.com")` дважды
**Ожидаемый результат:** Второй apply пропускается и возвращает `ALREADY_APPLIED`
