...
**Test ID:** `PIPE-019`
**Что проверяет:** Auto-hostlist v2-only: без legacy типов (`BlockageSignals/GetSignals`)
...
- Домен добавляется на v2 inspection signals
...
**Ожидаемый результат:** Строки не содержат переводы строк, присутствует префикс `[V2]`
...
**Test ID:** `DPI2-025`
**Что проверяет:** Guard: в v2 runtime-пути нет legacy `GetSignals(...)`/`BlockageSignals`
**Для чего:** Не допустить незаметного возврата legacy источника фактов в v2 цепочку
...
**Test ID:** `REG-009`
**Что проверяет:** Apply-транзакция содержит v2 контракт (request/snapshot/result/contributions)
...
**Test ID:** `REG-012`
**Что проверяет:** Операции ручного применения v2-плана сериализованы (нет параллельного apply)
