...
**Описание:**
Добавить лог‑метрики для Flow‑слоя: моменты открытия/первых событий, количество flow’ов, процент целевых flow’ов и конфигурируемый фильтр, чтобы понимать, насколько Flow полезен и когда он успевает «поднять глаза».
...
- Расширенное логирование в `TrafficAnalyzer`:
  - таймстемпы открытия Flow handle и первого события для целевого PID,
  - суммарное количество Flow‑событий и доля приходящихся на целевой PID,
  - настройка `FlowFilter` (например, через конфиг или профиль, а не жёстко `"true"`).
