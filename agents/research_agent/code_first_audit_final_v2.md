...
- **Профиль:** Загружается "Default" (из `Profiles/Default.json`), но **игнорируется** основной логикой (см. п. 2.2).
...
2. **Material Design:**
    ...
    - *Вердикт:* **Подключить Material Design.** Текущая реализация на кастомных стилях сложна в поддержке и не соответствует документации. Внедрение MD улучшит UI/UX без заметного влияния на производительность.
...
2.  **Мертвый механизм профилей:**
    - `Program.cs` загружает `Profiles/Default.json` в `Program.Targets`.
    - Однако `DiagnosticOrchestrator` и `LiveTestingPipeline` работают в режиме **Dynamic Mode** (сниффинг трафика) и **не используют** этот список целей. Метод `Config.ResolveTargets()` имеет 0 использований.
    - *Вердикт:* Механизм профилей — рудимент старой архитектуры.

3.  **`MainViewModelRefactored`:**
...
1. **Documentation Purge:**
    - Вычистить все упоминания CLI из `copilot-instructions.md`, `README.md`, `ARCHITECTURE_CURRENT.md`.
    - Актуализировать описание архитектуры (только GUI).

2. **Code Cleanup & Enhancement:**
    - **Подключить MaterialDesignInXaml** и перевести UI на него.
    ...
    - (Опционально) Удалить код загрузки профилей из `Program.cs` и `Config.cs`, раз он не используется.
