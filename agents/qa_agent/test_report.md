...
- FAIL (по логам): текущий Gate-лог в UI — это агрегированная строка вида `[V2][GATE1] hostKey=... recentTypes=...`, без порядка событий/таймлайна. Это позволяет увидеть “какие типы были”, но не даёт восстановить именно цепочку “что раньше/что потом” (требование “HostTested → потом …”) по одному только логу.
...
3) В логе UI найти строки `[V2][GATE1]`.
...
4) Для каждого проблемного хоста зафиксировать v2 хвост в строке вида `(... [V2] диагноз=... уверенность=...% ...)` и убедиться, что диагноз не `Unknown` и уверенность > 30.
...
1) Gate 1→2: “цепочки восстанавливаются по логам” — текущий формат `[V2][GATE1] ... recentTypes=...` не содержит порядка/таймлайна событий (только множество типов). Это может не проходить критерий “восстановить цепочку HostTested → потом …”.
	- Как увидеть: запустить диагностику и посмотреть строки `[V2][GATE1]` — там нет времени/порядка.
