# [PURPLE] Task Owner: DPI Intelligence v2 (Phase 2) — MVP Signals → Diagnosis → Strategy (без авто-применения)

## Проблема
Сейчас обход работает нестабильно и рекомендует нерелевантные действия; пользователю непонятно, что делать. Если бы текущая схема работала — мы бы не обсуждали внедрение v2.
...
## Цель (MVP)
Внедрить объяснимый слой DPI Intelligence v2:
...
## Definition of Done
- В коде существует слой v2 с границами: SignalsAdapterV2 → DiagnosisEngineV2 → StrategySelectorV2 → ExecutorMvp (логирование).
...
- Legacy не удаляется первым шагом, но используется только справочно и не управляет рекомендациями v2.
