...
2. **DI внедрён частично**: `App` использует `Microsoft.Extensions.DependencyInjection` (composition root), регистрации вынесены в `Utils/ServiceCollectionExtensions.cs`; часть зависимостей (например, `NoiseHostFilter`) уже прокидывается через конструкторы, а WPF-конвертеры резолвят `NoiseHostFilter` через `App` (с fallback на `NoiseHostFilter.Instance`), но граф объектов внутри `MainViewModel` пока создаётся вручную.
