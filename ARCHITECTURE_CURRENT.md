...
*   **`Windows/OperatorWindow.xaml`**: упрощённый «операторский» интерфейс (status-first + одна CTA + откат).
    ...
    *   OperatorWindow реализован как упрощённый wizard:
        *   Шаг 1: выбор цели (сервис/игра). Выбор выставляет `OutcomeTargetHost` (контекст автопилота/селективных стратегий) и сохраняется best-effort в `state\operator_target.json` через `Utils/OperatorTargetStore`.
        ...
        *   Шаг 4: итог (сводка счётчиков + один главный CTA).
