...
*   **`BypassController`**: ViewModel, отвечающая за настройки обхода.
    ...
    *   P0.1 (MVP, наблюдаемость): ведёт журнал транзакций «применения обхода» (hostKey + groupKey + candidate IP endpoints + applied strategy + activation + policy snapshot) и показывает его в UI (вкладка «Применение») с экспортом JSON в `artifacts/` и копированием в буфер.
