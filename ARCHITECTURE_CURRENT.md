...
*   **`MainWindow.xaml`**: Главное окно приложения. Использует библиотеку `MaterialDesignInXaml` для визуализации карточек с проблемами.
    ...
    *   Доменное применение доступно прямо в строке (кнопка "🌐 Домен" при наличии подсказки). «Детали применения обхода» доступны в окне деталей по double-click: JSON последней apply-транзакции группы (плюс participation snapshot, включая attachments per-hostKey) и кнопка копирования.
