...
Категория `bypass` (TLS bypass) частично покрыта детерминированными smoke-тестами `BYPASS-003..004` в `TestNetworkApp/Smoke/SmokeTests.Bypass.cs`: сбор метрик через событие `MetricsUpdated` и вычисление вердикта через `VerdictChanged` по порогам ratio RST/фрагментации. Для детерминизма используется `BypassFilter` + синтетические TCP пакеты (ClientHello/RST), а также internal-хуки `TlsBypassService` для принудительного чтения метрик без запуска WinDivert.
