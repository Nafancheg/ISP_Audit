...
2. **DI внедрён частично**: `App` использует `Microsoft.Extensions.DependencyInjection` (composition root), регистрации вынесены в `Utils/ServiceCollectionExtensions.cs`; часть зависимостей (например, `NoiseHostFilter`) уже прокидывается через конструкторы, но граф объектов внутри `MainViewModel` пока создаётся вручную.
