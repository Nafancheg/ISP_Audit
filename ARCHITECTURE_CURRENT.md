...
2. **DI внедрён частично**: `App` использует `Microsoft.Extensions.DependencyInjection` (composition root), регистрации вынесены в `Utils/ServiceCollectionExtensions.cs`; критичные shared-сервисы (`NoiseHostFilter`, `ITrafficFilter`/`UnifiedTrafficFilter`, `AutoHostlistService`) идут как singleton. `TrafficCollector`/`LiveTestingPipeline` больше не создают фильтр сами — `ITrafficFilter` передаётся явно. При этом часть графа внутри `MainViewModel`/оркестратора всё ещё создаётся вручную.
