...
#### Модель B (Design target): per-card «закрепление обхода» + прозрачная транзакция применения

Контекст: пользователь принимает решения **на уровне карточки** (hostKey/target), поэтому «применение обхода» должно быть:
- привязано к конкретной карточке (кто инициировал и для какой цели);
- сериализовано (чтобы не было гонок и эффекта «переключателя»);
- наблюдаемо (видно, что именно применено и к чему).
...
- UX/семантика становится per-card: каждая карточка может иметь своё «закрепление обхода» (attachment) и последнюю транзакцию применения.

##### Правило конкуренции (рекомендуемое по умолчанию)

**Один активный владелец обхода в момент времени (single-owner attach).**

Причины:
- Это отражает физическую реальность текущей архитектуры: фильтры/движок применяются глобально в рамках процесса.
- Это радикально упрощает объяснимость: в UI всегда можно показать «сейчас активна конфигурация от карточки X».
- Это минимизирует риск неконтролируемого расширения blast radius (если начать объединять правила нескольких целей).

Поведение:
- Карточки могут хранить «желаемую» закреплённую конфигурацию (attachment), но активной становится только одна.
- При новом «применении обхода» активный владелец меняется, а UI явно показывает смену владельца (без скрытых переключений).

Опционально (после стабилизации): мульти-attach возможен только как отдельный режим, где объединение правил безопасно и явно показано (например, объединение endpoint-наборов для селективного QUIC→TCP), но это не базовая семантика.
...
- Request: hostKey, инициатор (ручное действие / автоматическое правило), причина, таймаут/отмена.
- Snapshot: что реально применили (стратегия/assist-флаги), на какие endpoint-ы нацелились (IPv4/IPv6, ports, proto), какие фильтры активны (имя + приоритет), на каких входных данных построено (DNS/SNI/resolve, возраст/TTL).
- Result: Applied/Failed/RolledBack + ошибка + тайминги.

##### Отражение в UI/логах

- В карточке: краткое резюме «закреплено/не закреплено», статус последней транзакции, и кто сейчас владелец активной конфигурации.
- В деталях (панель/expander): полный снимок последней транзакции и кнопка «Скопировать» (для репорта).
- В логе: компактная строка на транзакцию (TransactionId + hostKey + strategy + flags + endpoint-count) и возможность разворота деталей.
