...
#### Модель B (Design target): Accumulative Attachment Model (группа целей = одна конфигурация обхода)
...
**Accumulative Attachment Model:** группа целей держит набор attachments, а итоговая конфигурация обхода строится как объединение вкладов.
...
- Стратегия TLS-обхода: одна на весь движок/сессию; если вклады конфликтуют, выбирается **доминирующая/самая сильная** стратегия (например, Disorder доминирует Fragment), и это явно отображается как “GroupEffectiveStrategy”.
...
- Snapshot:
    - EffectiveGroupConfig: итоговая стратегия/assist-флаги, итоговые endpoint-наборы, итоговая цель outcome.
    - Contributions: список вкладов (какие карточки/hostKey участвовали, какой endpoint-набор/флаги принесли).
