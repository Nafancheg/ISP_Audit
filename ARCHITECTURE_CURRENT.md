...
Категория `bypass` (TLS bypass) частично покрыта детерминированными smoke-тестами `BYPASS-003..011` в `TestNetworkApp/Smoke/SmokeTests.Bypass.cs`: сбор метрик через событие `MetricsUpdated`, вычисление вердикта через `VerdictChanged` по порогам ratio RST/фрагментации, а также проверки логики `BypassFilter` (фрагментация/дизордер сегментов, TTL Trick, Drop RST, гейтирование по 443+SNI и порог threshold для коротких ClientHello). Для детерминизма используется `BypassFilter` + синтетические TCP пакеты (ClientHello/RST) и захват отправленных фрагментов через тестовый sender, а `TlsBypassService` получает internal-хуки для принудительного чтения метрик без запуска WinDivert.
