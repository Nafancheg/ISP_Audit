...
*   **`Windows/OperatorWindow.xaml`**: упрощённый «операторский» интерфейс (status-first + одна CTA + откат).
    ...
    *   OperatorWindow реализован как упрощённый wizard:
        *   Шаг 1: выбор приложения (.exe) для наблюдения/диагностики. Якорь проблемы определяется по результатам и показывается в Operator как «Проблемный ресурс» (на базе `OutcomeTargetHost`).
        *   Шаг 2: выбор источника трафика (быстрая проверка / выбранное приложение .exe).
        *   Шаг 3: прогресс диагностики (индикатор + «проверено X из Y»).
        *   Шаг 4: итог (сводка + «проверено N / проблемных M» и карточки проблем по умолчанию) + один главный CTA.
        *   Шаг 5: исправление (Apply) + пост‑проверка (post‑apply ретест) + краткий итог «стало лучше/частично/не помогло/не удалось проверить».
        *   Отображение шагов управляется операторским состоянием в `OperatorViewModel`.

    *   Минималистичный header: слева название приложения, справа иконки ⚙️ (Operator-safe настройки) и ? (краткая справка).
        *   ⚙️ открывает `Windows/OperatorSettingsWindow.xaml` (без инженерных тумблеров; только безопасные параметры Operator + переключение темы).
        *   ? открывает `Windows/OperatorHelpWindow.xaml` (как пользоваться + действие «Открыть расширенный режим»).
...
P1.11 (Operator UX): быстрый откат «Откатить» откатывает **только изменения Autopilot**: восстанавливает последний зафиксированный snapshot ручного apply (по apply-транзакциям), чтобы не ломать ручные настройки Engineer. Маршрут: `OperatorViewModel.RollbackCommand` → `BypassController.RollbackAutopilotOnlyAsync`. Полный откат (включая DNS/DoH при наличии backup) остаётся отдельным путём: `BypassController.RollbackAllAsync`.

P1.11 (Engineer UX): есть явная кнопка «Снять всё (включая ручное)» с подтверждением, которая выполняет полный откат: `MainWindow` → `MainViewModel.EngineerResetAllBypassCommand` → `BypassController.RollbackAllAsync`.
...
* Ручное применение плана (без auto-apply): `LiveTestingPipeline` публикует объектный `BypassPlan` через событие `OnPlanBuilt`, `DiagnosticOrchestrator` хранит последний план и применяет его только по клику пользователя через `BypassController.ApplyIntelPlanAsync(...)`. Исполнение apply/timeout/rollback вынесено в `Core/Bypass/BypassApplyService`.
    * P1.5: критические секции операций оркестратора (cts/pipeline/collector start/stop/dispose и обвязка apply) сериализуются, чтобы быстрые клики Start/Cancel/Retest/Apply не создавали две активные операции и не приводили к гонкам.
