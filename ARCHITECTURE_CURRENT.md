...
*   **`MainWindow.xaml`**: Главное окно приложения. Использует библиотеку `MaterialDesignInXaml` для визуализации карточек с проблемами.
    ...
    *   После применения обхода ("Подключить"/Apply) UI запоминает и показывает **фактически применённую** стратегию для группы/строк результата (поверх рекомендаций). Пометка **APPLIED** работает аккумулятивно: несколько групп могут быть отмечены одновременно. Шумовые хосты помечаются как **EXCLUDED**. Поддерживается ручное участие карточек в группе: бейдж **OUT** и кнопка исключения/возврата (participation). Manual participation сохраняется в `%LocalAppData%\\ISP_Audit\\group_participation.json` и влияет на UI-маркеры (исключённые карточки не помечаются APPLIED и не получают AppliedBypassStrategy).
    *   Доменное применение доступно прямо в строке (кнопка "🌐 Домен" при наличии подсказки). Для выбранной строки доступна панель «Детали применения обхода» с JSON-снимком последней apply-транзакции группы (плюс participation snapshot) и кнопкой копирования в буфер.
