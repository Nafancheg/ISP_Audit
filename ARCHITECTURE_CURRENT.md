...
Важно (runtime, v2): для пресета `v2` фильтр `BypassFilter` принудительно глушит QUIC/HTTP3, отбрасывая исходящий UDP на порт 443 (`Udp443Dropped++`). Это сделано намеренно, потому что TLS-обход влияет только на TCP, а YouTube и многие сайты по умолчанию используют QUIC.
