...
1. **Глобальное состояние**: снижено (static singleton API `NoiseHostFilter` удалён), но остаются другие источники SSoT/фабрики (`BypassStateManager.GetOrCreate` и т.п.).
2. **DI внедрён частично**: `App` использует `Microsoft.Extensions.DependencyInjection` (composition root), регистрации вынесены в `Utils/ServiceCollectionExtensions.cs`; shared-сервисы (`NoiseHostFilter`, `ITrafficFilter`/`UnifiedTrafficFilter`, `AutoHostlistService`, а также `TrafficEngine`/`BypassStateManager` и слой VM: `BypassController`/`DiagnosticOrchestrator`/`TestResultsManager`) идут как singleton. Для поддержки XAML-ресурсов (конвертеры) контейнер строится до загрузки ресурсов, чтобы не было fallback-инстанцирования через `new`. `TrafficCollector`/`LiveTestingPipeline` больше не создают фильтр сами — `ITrafficFilter` передаётся явно. Остаются области, где всё ещё создаются объекты вручную (например, часть вспомогательных UI-объектов и отдельные компоненты pipeline) — миграция продолжается.
