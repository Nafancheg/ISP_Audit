...
- Gate проверяется динамически в runtime (без latched bool в конструкторах сервисов), поэтому freeze не «залипает» между runs: после снятия ClassicMode авто-обновления/sync снова разрешаются на следующем run.
...
При `ISP_AUDIT_CLASSIC_MODE=1` `ReactiveTargetSyncService` работает в режиме observe-only (входящие сигналы не приводят к мутациям execution-state в рамках run), а быстрый runtime refresh селективных QUIC targets также не выполняет мутаций auto-add.
