...
*   **`MainWindow.xaml`**: Главное окно приложения. Использует библиотеку `MaterialDesignInXaml` для визуализации карточек с проблемами.
    ...
    *   Manual participation: бейдж **OUT** и кнопка исключения/возврата. Сохраняется в `%LocalAppData%\\ISP_Audit\\group_participation.json` и влияет на UI-маркеры (исключённые карточки не помечаются APPLIED и не получают AppliedBypassStrategy).
    *   P0.1 Step 11 (gate): дополнительно сохраняется pinning `hostKey -> groupKey` (v2 в том же `group_participation.json`) и используется «стабильный» groupKey в UI/VM, чтобы изменения `SuggestedDomainSuffix` во время диагностики не могли бесшумно поменять ручной состав группы.
    *   Доменное применение доступно прямо в строке (кнопка "🌐 Домен" при наличии подсказки). «Детали применения обхода» доступны в окне деталей по double-click: JSON последней apply-транзакции группы (плюс participation snapshot) и кнопка копирования.
