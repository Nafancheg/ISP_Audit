...
#### Bad Checksum — что это значит
`Bad Checksum` применяется **только к фейковым пакетам** (TTL=1) в рамках техник TLS обхода.
Это сделано намеренно: такие пакеты не предназначены для доставки до сервера и используются как «шум/приманка» на первом хопе.
