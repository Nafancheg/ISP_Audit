...
По результатам диагностики формируются рекомендации (в логе помечаются как `[INTEL]`). Оптимизация включается вручную — через тумблеры или кнопкой «Применить рекомендации» (доступно только при запуске от администратора).
...
- Устанавливает соединения к заранее заданному набору известных хостов
- Делает один цикл запросов и завершает работу (удобно для воспроизводимых проверок)
...
Если диагностика выявила проблемы, откройте панель оптимизации и примените рекомендации (или включите нужные тумблеры вручную).

### Сценарий воспроизведения (P0.4: TrafficEngine Apply/Disable)

Цель: воспроизводимо нагрузить цепочку Apply/Disable (WinDivert/TrafficEngine) в GUI, чтобы поймать редкие зависания/краши.

**Профиль**:
- Используется профиль по умолчанию `Default` (загружается автоматически при старте).

**Цель (браузер)**:
- `msedge.exe` или `chrome.exe` (любой Chromium-подобный браузер).
...
4. В браузере откройте 3–5 вкладок с HTTPS-сайтами и обновите их несколько раз (важно, чтобы был активный TLS-трафик).
...
6. Выполняйте циклы **Apply → Disable** (или включение/выключение обхода) с частотой **~3–5 циклов/сек** в течение **30–60 секунд**.
  - Если вы проверяете именно «быстрый клик» сценарий — держите темп максимально стабильным.

**Что сохранить при зависании/краше**:
- Лог: `Logs/isp_audit_vm_*.log` (в идеале целиком) и минимум ±100 строк вокруг последних сообщений `[Bypass]`/`[TrafficEngine]`/`Loop crashed`.
- Транзакции Apply/Disable (если включены в билде): `state\\apply_transactions.json` и любые экспортированные артефакты из `artifacts/`.

Примечание: для автоматизированного стресс‑прогона без GUI используйте strict smoke `INFRA-010` (см. раздел про smoke-тесты ниже).
