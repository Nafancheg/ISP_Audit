...
├─────────────────────────────────────────────────┤
│                                                 │
│  [📁 Обзор]  C:\Games\FsHud\FsHud.exe          │
│                                                 │
│  [▶ Запустить диагностику]                     │
│                                                 │
│  ┌─ Результаты ────────────────────────────┐   │
│  │ ✓ auth.fshud.com:443         (54ms)     │   │
│  │ ✓ api.fshud.com:443          (65ms)     │   │
│  │ ✓ www.google.com:443         (58ms)     │   │
│  │ ❌ googleads.g.doubleclick.net:443      │   │
│  │    DNS:✓ TCP:✓ TLS:✗ | TLS_DPI          │   │
│  │    💡 Рекомендация: DROP_RST            │   │
│  │ ✓ analytics.google.com:443   (42ms)     │   │
│  └──────────────────────────────────────────┘   │
│                                                 │
│  Соединений: 51 | Тестов: 48 ✓ / 3 ❌          │
└─────────────────────────────────────────────────┘
```
...
└──────────┬──────────┘  └──────────┬──────────┘  └─────────────────────┘
           ...
           ▼             ▼                                     ▼
┌──────────────────┐  ┌──────────────────────────────┐  ┌──────────────────┐
│WinDivertBypass-  │  │      Мониторинг трафика      │  │LiveTestingPipeline│
│    Manager       │  │                              │  │                  │
│                  │  │ ┌────────────────────────┐   │  │ Tester →         │
│ • RST Blocker    │  │ │  ConnectionMonitor     │   │  │ Classifier →     │
│ • TLS Fragment   │  │ │  (Socket Layer -1000)  │   │  │ BypassCoord →    │
│ • Fake TTL       │  │ └───────────┬────────────┘   │  │ UI               │
│                  │  │             │                │  │                  │
│  Priority: 200   │  │ ┌───────────▼────────────┐   │  │                  │
└──────────────────┘  │ │    NetworkMonitor      │   │  └────────▲─────────┘
