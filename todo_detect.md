...
- [x] Дальнейшее развитие агрегатора сигналов:
...
- [x] Логика детекции (следующие шаги):
...
- [x] Интеграция с классификатором и UI:
...
- [x] Создать модуль `RstInspectionService`:
...
## 6. План дальнейших действий (Next Steps) - Updated 09.12.2025

### 6.1. Улучшение эвристик (Tuning)
- [x] **Умный подсчет ретрансмиссий**:
  - Модифицировать `TcpRetransmissionTracker` для подсчета *общего* количества пакетов в потоке.
  - В `BlockageSignals` заменить жесткий порог `> 3` на относительный (например, `> 5%` ретрансмиссий при общем числе пакетов > 10).
- [ ] **Уточнение HTTP редиректов**:
  - Проверить на практике работу проверки SLD (Second Level Domain) для исключения ложных срабатываний на CDN редиректах.

### 6.2. Реализация недостающих стратегий
- [x] **TTL_TRICK**:
  - Добавить поддержку `TTL_TRICK` в `BypassProfile` и `BypassFilter` (манипуляция IP TTL для исходящих пакетов).
  - Включить эту стратегию в рекомендации для `HTTP_REDIRECT_DPI`.

### 6.3. UX "Авто-тюнинга" (Safe Mode)
- [x] **Индикация в UI**:
  - Если `StandardBlockageClassifier` возвращает `TCP_RST_INJECTION` или `HTTP_REDIRECT_DPI`, выводить в UI специальную кнопку/предложение "Включить безопасный режим" (через стратегию `SAFE_MODE`).
- [x] **Логика Safe Mode**:
  - Реализовать команду, которая принудительно включает `DROP_RST` + `TLS_DISORDER` (как наиболее универсальную комбинацию) для текущей цели.

### 6.4. Валидация (QA)
- [ ] **Проверка Facebook**:
  - Убедиться, что блокировка QUIC (UDP/443) приводит к статусу `UDP_BLOCKAGE` и рекомендации проверить TCP/TLS.

### 6.5. Выполнено (Done)
- [x] Визуализация в UI (бейджи для RST/Redirect).
- [x] Поддержка UDP/QUIC (`UdpInspectionService`).
- [x] Экспорт данных в JSON.
- [x] Фильтрация `0.0.0.0`.
- [x] Отображение стратегии в UI.
