...
- [ ] Перенести отображение новых сигналов (`TCP_RST_INJECTION`, `HTTP_REDIRECT_DPI`) из текстового лога в красивые бейджи/иконки на карточках `TestCard.xaml`.
- [ ] Добавить график/индикатор "здоровья" соединения (fail rate за последнюю минуту).

### 6.2. Поддержка UDP/QUIC
- [ ] Реализовать `UdpInspectionService` для анализа блокировок UDP (DTLS handshake drops), критично для Star Citizen.
- [ ] Добавить поддержку QUIC (анализ Initial пакетов).
