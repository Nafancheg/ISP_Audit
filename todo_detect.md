...
## 6. План дальнейших действий (Next Steps)

### 6.1. Визуализация в UI
- [x] Перенести отображение новых сигналов (`TCP_RST_INJECTION`, `HTTP_REDIRECT_DPI`) из текстового лога в красивые бейджи/иконки на карточках `TestCard.xaml`.
- [x] Добавить график/индикатор "здоровья" соединения (fail rate за последнюю минуту).

### 6.2. Поддержка UDP/QUIC
- [x] Реализовать `UdpInspectionService` для анализа блокировок UDP (DTLS handshake drops), критично для Star Citizen.
- [x] Добавить поддержку QUIC (анализ Initial пакетов).

### 6.3. Авто-тюнинг стратегий
- [ ] Если детектирован `TCP_RST_INJECTION`, предлагать пользователю "Безопасный режим" с автоматическим включением `DROP_RST`.
- [ ] Если детектирован `HTTP_REDIRECT_DPI`, предлагать `FAKE_REQUEST` или `TTL_TRICK`.

### 6.4. Экспорт данных
- [x] Включить детальную статистику (флаги детекции RST, Redirect, Packet Loss, UDP) в сохраняемый JSON-отчет для оффлайн-анализа.

### 6.5. Исправление известных проблем (Bug Fixes)
- [x] Фильтрация `0.0.0.0` в `TrafficCollector` (исключение биндинг-сокетов).
- [x] Отображение стратегии в UI: парсинг "💡 Рекомендация:" в `TestResultsManager` для заполнения столбца "Стратегия".
- [x] (Low Priority) Дедупликация `www.` хостов (объединение `youtube.com` и `www.youtube.com`).
- [ ] (Investigation) Facebook "OK" но не грузится: вероятно QUIC или HTTP-блокировка (требует реализации п. 6.2).
