...
namespace IspAudit.Utils
{
    ...
    /// ИСПОЛЬЗУЕТСЯ ДЛЯ STAGE2: детальный анализ конкретных целей с захватом пакетов.
    /// Для Stage1 (поиск IP-адресов) используйте TrafficAnalyzer (Flow-only).
    /// </summary>
    internal static class TrafficAnalyzerDualLayer
    {
        ...
        private static void TryParseDns(byte[] buffer, int length, ConcurrentDictionary<string, string> dnsCache)
        {
             ...
             // Вызов метода из текущего класса
             TrafficAnalyzerDualLayer.TryParseDnsResponse(buffer, dnsOffset, length, dnsCache);
        }
