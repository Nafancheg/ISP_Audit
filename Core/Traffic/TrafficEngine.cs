        ...
        public void Dispose()
        {
            try
            {
                ...
                _ = stopTask.ContinueWith(t =>
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"[TrafficEngine] CTS dispose failed (continuation): {ex.GetType().Name}; {ex.Message}; hresult={ex.HResult}");
                    }
                }, TaskScheduler.Default);
            }
            catch (Exception ex)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"[TrafficEngine] CTS dispose failed (fallback): {disposeEx.GetType().Name}; {disposeEx.Message}; hresult={disposeEx.HResult}");
                }
            }
        }
    }
